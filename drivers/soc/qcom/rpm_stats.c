...
#define TIMEOUT_FOR_DUMP (3*60*60)
...
void rpmhstats_statistics(void)
{
	...
	for (i = 0; i < pdata->num_records; i++) {
		if (time != 0) {
				...
				if (time_since_last_mode > TIMEOUT_FOR_DUMP) {
					...
					actual_last_sleep =
						get_time_in_msec(
							data->accumulated);
					pr_err("Count:%d\n\t", data->count);
					pr_err("time in last mode(ms):%llu\n",
						time_in_last_mode);
					pr_err("time since last mode(s):%llu\n",
						time_since_last_mode);
					pr_err("actual last sleep(ms):%llu\n\n",
						actual_last_sleep);
					//get subsystem stats
					if (get_apps_stats(false) == false) {
						pr_err("The issue is APPS");
						continue;
					} else {
						get_subsystem_stats(false);
						panic("RPMH not entering PC");
					}
				} else if (arch_counter_get_cntvct() - time >
					TIMEOUT_FOR_DUMP)
					time = 0;
			} else if (arch_counter_get_cntvct() - time >
					TIMEOUT_FOR_DUMP)
			...
			time = arch_counter_get_cntvct();
			data[i].stat_type = msm_rpmstats_read_long_register(
					reg, i,
			...
			data[i].count = msm_rpmstats_read_long_register(reg, i,
					offsetof(struct msm_rpm_stats_data,
					count));
			data[i].last_entered_at =
					msm_rpmstats_read_quad_register(reg,
					i, offsetof(struct msm_rpm_stats_data,
					last_entered_at));
			data[i].last_exited_at =
					msm_rpmstats_read_quad_register(reg,
					i, offsetof(struct msm_rpm_stats_data,
					last_exited_at));
			data[i].accumulated = msm_rpmstats_read_quad_register(
					reg, i, offsetof(
					struct msm_rpm_stats_data,
					accumulated));
			get_apps_stats(true);
			...
			struct kobj_attribute *attr, char *buf)
{
	...
	length = scnprintf(buf, 480, "%s", prvdata.buf);
	return length;
	}

...
static   uint64_t   LrmeaRequreCount;
static   uint64_t   LrmeaReleaseCount;
	...
	struct cam_acquire_dev_cmd *acquire)
{
	...
	if (!strcmp(node->name, "cam-lrme")) {
		CAM_ERR(CAM_CORE, "Acquire device for node %s session_handle=%d,%lld",
			node->name, acquire->session_handle,
			++LrmeaRequreCount);
	}
	...
	if (!ctx) {
		rc = -ENOMEM;
		if (!strcmp(node->name, "cam-lrme"))
			CAM_ERR(CAM_CORE, "Can not get context for node %s",
				node->name);
		goto err;
	}
